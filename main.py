...
async def main(page: ft.Page):
    ...
    # WAŻNE DLA MOBILE: Wyłączamy domyślne scrollowanie strony, 
    # abyśmy mogli sami zarządzać scrollem czatu w kontenerze.
    page.scroll = False 
    page.padding = 10
    ...
    # Czat - to on będzie się scrollował
    chat_col = ft.Column(scroll="auto", auto_scroll=True, expand=True, spacing=5)
    
    # Input (Wiadomość + Wyślij)
    ...
    row_input = ft.Row([input_chat, btn_send], alignment="spaceBetween")
    
    # Przyciski
    btn_bid = ft.FilledButton("+100", disabled=True, expand=1)
    btn_pass = ft.ElevatedButton("Pas", disabled=True, expand=1)
    btn_allin = ft.FilledButton("VB", style=ft.ButtonStyle(bgcolor="red"), disabled=True, expand=1)
    ...
    # --- GŁÓWNY LAYOUT (Responsive) ---
    # Używamy Column z expand=True, żeby zajął cały ekran.
    # Wewnątrz, Container z chat_col ma expand=True, więc to on zajmie resztę miejsca.
    # Dzięki temu row_input i przyciski będą zawsze na dole (nad klawiaturą).
    ...
    layout = ft.Column([
        ...
        # Kontener czatu (rozciągliwy)
        ft.Container(
            content=chat_col,
            border=ft.border.all(1, "grey"), 
            border_radius=10, 
            padding=5,
            expand=True # To jest kluczowe dla mobile!
        ),
    ...
    ], spacing=5, expand=True) # Główna kolumna też ma expand
    ...
    async def render_chat(chat_list, players_list):
        ...
        # FIX: Usunięto sprawdzanie last_ts, żeby zawsze odświeżać (pewność 1. pytania)
        # Zamiast tego czyścimy i rysujemy. Flet Web jest szybki.
